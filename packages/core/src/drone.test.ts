...
test('get pathing telemetry data for multiple waypoint', async () => {
	...
	const bearing = 90;
	const waypoint = geolib.computeDestinationPoint(startLocation, distance, bearing);
	const waypoint2 = geolib.computeDestinationPoint(startLocation, distance * 10, bearing);
	...
	expect(res.length).toBeLessThanOrEqual(2500);
});

test('main for multiple waypoint', async () => {
	const startLocation = { latitude: 37.7749, longitude: -122.4194 };
	const device = 'test';
	const speed = 20;
	const altitude = 15;
